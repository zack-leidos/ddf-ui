...
export var CesiumDrawings = function (_a) {
    ...
    */
    return (React.createElement(React.Fragment, null,
        filterModels.filter(isNotBeingEdited).map(function (model) {
            var drawMode = getDrawModeFromModel({ model: model });
            switch (drawMode) {
                case 'bbox':
                    return React.createElement(CesiumBboxDisplay, { key: model.cid, model: model, map: map });
                case 'circle':
                    return (React.createElement(CesiumCircleDisplay, { key: model.cid, model: model, map: map }));
                case 'line':
                    return React.createElement(CesiumLineDisplay, { key: model.cid, model: model, map: map });
                case 'poly':
                    return (React.createElement(CesiumPolygonDisplay, { key: model.cid, model: model, map: map }));
                default:
                    return React.createElement(React.Fragment, null);
            }
        }),
        models.filter(isNotBeingEdited).map(function (model) {
            var drawMode = getDrawModeFromModel({ model: model });
            switch (drawMode) {
                case 'bbox':
                    return React.createElement(CesiumBboxDisplay, { key: model.cid, model: model, map: map });
